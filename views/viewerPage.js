...
<body>
  <div id="app">
    ...
    <div id="sidebar">
      <div id="flightDashboard" style="margin-bottom:16px; padding:12px; border:1px solid #ddd; border-radius:8px; background:#fff;">
        <h3 style="margin:0 0 10px 0;">Flight Status</h3>

        <div class="meta"><strong>Speed:</strong> <span id="dashSpeed">--</span></div>
        <div class="meta"><strong>Heading:</strong> <span id="dashHeading">--</span></div>
        <div class="meta"><strong>Altitude:</strong> <span id="dashAltitude">--</span></div>
        <div class="meta"><strong>Status:</strong> <span id="dashStatus">--</span></div>
      </div>
      <h2>Flight Viewer</h2>
...
function smoothHeadingDegrees(previousDeg, nextDeg, alpha = 0.15) {
  ...
  const delta = shortestAngleDegrees(previousDeg, nextDeg);
  return normalizeHeadingDegrees(previousDeg + delta * alpha);
}

function getHeadingOnlyTransform(position, headingDeg) {
  const enu = Cesium.Transforms.eastNorthUpToFixedFrame(position);
  const heading = Cesium.Math.toRadians(headingDeg);
  const rotation = Cesium.Matrix3.fromRotationZ(-heading);
  return Cesium.Matrix4.multiplyByMatrix3(enu, rotation, new Cesium.Matrix4());
}

function setStableCameraView(cameraPosition, targetPosition) {
  const direction = Cesium.Cartesian3.normalize(
    Cesium.Cartesian3.subtract(targetPosition, cameraPosition, new Cesium.Cartesian3()),
    new Cesium.Cartesian3()
  );

  const enu = Cesium.Transforms.eastNorthUpToFixedFrame(targetPosition);
...
function computeVerticalSpeedMetersPerSecond(a, b) {
  if (!a || !b) return 0;
  ...
  const alt1 = Number.isFinite(a.alt) ? a.alt : 0;
  const alt2 = Number.isFinite(b.alt) ? b.alt : 0;

  return (alt2 - alt1) / dtSeconds;
}

function isLandedState({ aglM, speedMps, verticalSpeedMps, pitchDeg, rollDeg }) {
  return (
    Number.isFinite(aglM) &&
    aglM < 8 &&
    Math.abs(verticalSpeedMps) < 1.5 &&
    speedMps < 85 &&
    Math.abs(pitchDeg) < 8 &&
    Math.abs(rollDeg) < 10
  );
...
async function applyLowAltitudeTerrainClearance(points, viewer, options = {}) {
  const lowAltitudeThresholdM = options.lowAltitudeThresholdM ?? 120;
  const minClearanceM = options.minClearanceM ?? 3;

  if (!viewer || !viewer.scene || !Array.isArray(points) || points.length === 0) {
    return points || [];
  }
  ...
  for (let i = 0; i < adjusted.length; i += 1) {
    const alt = Number.isFinite(adjusted[i].alt) ? adjusted[i].alt : 0;

    if (alt <= lowAltitudeThresholdM) {
      positions.push(Cesium.Cartographic.fromDegrees(adjusted[i].lon, adjusted[i].lat));
      indexMap.push(i);
    }
  }
  ...
  try {
    ...
    console.warn("[terrain clearance] terrain sampling failed", err);
    return adjusted;
  }

  for (let j = 0; j < sampled.length; j += 1) {
    const sampledHeight = sampled[j] && Number.isFinite(sampled[j].height) ? sampled[j].height : undefined;
    if (!Number.isFinite(sampledHeight)) {
      continue;
    }

    const pointIndex = indexMap[j];
    const rawAlt = Number.isFinite(adjusted[pointIndex].alt) ? adjusted[pointIndex].alt : 0;

    adjusted[pointIndex].alt = Math.max(rawAlt, sampledHeight + minClearanceM);
  }
...
function updateDashboard({ speedMps, headingDeg, altitudeM, statusText }) {
  const speedEl = document.getElementById("dashSpeed");
  const headingEl = document.getElementById("dashHeading");
  const altitudeEl = document.getElementById("dashAltitude");
  const statusEl = document.getElementById("dashStatus");

  if (speedEl) speedEl.textContent = formatKnots(speedMps);
  if (headingEl) headingEl.textContent = formatHeadingDegrees(headingDeg);
  if (altitudeEl) altitudeEl.textContent = formatFeet(altitudeM);
  if (statusEl) statusEl.textContent = statusText || "--";
}

async function updateFlightDashboard() {
  ...
  if (idx === -1) {
    idx = Math.max(0, currentTrackPoints.length - 2);
  }

  const a = currentTrackPoints[idx];
  const b = currentTrackPoints[Math.min(idx + 1, currentTrackPoints.length - 1)];
  ...
  const pitchDeg = computePitchDegrees(a, b);
  const rollDeg = computeApproxRollDegrees(prevPoint, a, b);

  let altitudeM = NaN;
  let aglM = NaN;

  const currentPosition = currentEntity.position.getValue(viewer.clock.currentTime);
  if (currentPosition) {
    const cartographic = Cesium.Cartographic.fromCartesian(currentPosition);
    altitudeM = cartographic ? cartographic.height : NaN;

    try {
      const sampled = await Cesium.sampleTerrainMostDetailed(
        viewer.scene.terrainProvider,
        [Cesium.Cartographic.fromRadians(cartographic.longitude, cartographic.latitude)]
      );

      const terrainHeight = sampled[0] && Number.isFinite(sampled[0].height) ? sampled[0].height : NaN;
      if (Number.isFinite(terrainHeight) && Number.isFinite(altitudeM)) {
        aglM = altitudeM - terrainHeight;
      }
    } catch (err) {
      // ignore terrain sampling failure
    }
  }

  const landed = isLandedState({
    aglM,
    speedMps,
    verticalSpeedMps,
    pitchDeg,
...
function computeSpeedMetersPerSecond(a, b) {
  if (!a || !b) {
    return 0;
  }
  ...
  const p1 = Cesium.Cartesian3.fromDegrees(a.lon, a.lat, a.alt || 0);
  const p2 = Cesium.Cartesian3.fromDegrees(b.lon, b.lat, b.alt || 0);

  const distance = Cesium.Cartesian3.distance(p1, p2);
  return distance / dtSeconds;
}

function isGroundedPoint(prevPoint, point, nextPoint) {
  const alt = point && Number.isFinite(point.alt) ? point.alt : 0;

  const speedA = prevPoint ? computeSpeedMetersPerSecond(prevPoint, point) : 0;
  const speedB = nextPoint ? computeSpeedMetersPerSecond(point, nextPoint) : 0;
  const speed = Math.max(speedA, speedB);

  // Tune these as needed
  const lowAltitude = alt < 50;
  const slowSpeed = speed < 80;

  return lowAltitude && slowSpeed;
}
...
function shortestAngleDegrees(fromDeg, toDeg) {
  let diff = normalizeHeadingDegrees(toDeg) - normalizeHeadingDegrees(fromDeg);
...
function computeHeadingDegrees(a, b) {
  ...
  if (Cesium.Cartesian3.distance(p1, p2) < 1) {
    return 0;
  }

  const aCarto = Cesium.Cartographic.fromDegrees(a.lon, a.lat);
  const bCarto = Cesium.Cartographic.fromDegrees(b.lon, b.lat);
  const geodesic = new Cesium.EllipsoidGeodesic(aCarto, bCarto);

  return normalizeHeadingDegrees(Cesium.Math.toDegrees(geodesic.startHeading));
}

function computePitchDegrees(a, b) {
  const aCartesian = Cesium.Cartesian3.fromDegrees(a.lon, a.lat, a.alt || 0);
  const bCartesian = Cesium.Cartesian3.fromDegrees(b.lon, b.lat, b.alt || 0);

  const chordDistance = Cesium.Cartesian3.distance(aCartesian, bCartesian);
  if (!Number.isFinite(chordDistance) || chordDistance < 1) {
    return 0;
  }

  const dz = (b.alt || 0) - (a.alt || 0);
  return Cesium.Math.toDegrees(Math.atan2(dz, chordDistance));
}

function computeApproxRollDegrees(prevPoint, point, nextPoint) {
  if (!prevPoint || !nextPoint) {
    return 0;
  }

  const heading1 = computeHeadingDegrees(prevPoint, point);
  const heading2 = computeHeadingDegrees(point, nextPoint);

  const deltaHeading = shortestAngleDegrees(heading1, heading2);

  const t1 = new Date(prevPoint.time).getTime();
  const t2 = new Date(nextPoint.time).getTime();
  const dtSeconds = (t2 - t1) / 1000;
...
smoothedCameraHeadingDeg = smoothHeadingDegrees(
  ...
  headingAlpha
    ...
    setStableCameraView(cameraPosition, position);
  };
...
console.log("[renderFlight] start", {
  ...
  smoothedGroundHeadingDeg = null;

  if (dashboardHandler && viewer) {
    viewer.scene.preRender.removeEventListener(dashboardHandler);
...
if (!viewer.__cameraChangedInstalled) {
  ...
  const terrainAdjustedPoints = await applyLowAltitudeTerrainClearance(points, viewer, {
    lowAltitudeThresholdM: 120,
    minClearanceM: 3
  });
...
console.log("last time", points[points.length - 1].time);
  ...
  currentTrackPoints = terrainAdjustedPoints.slice();

  const animationPoints = densifyTrackPointsGeodesic(terrainAdjustedPoints, 30);
...
for (let i = 0; i < animationPoints.length; i += 1) {
  const prevPoint = i > 0 ? animationPoints[i - 1] : null;
  const point = animationPoints[i];
...
if (nextPoint) {
  headingDeg = computeHeadingDegrees(point, nextPoint);
  pitchDeg = computePitchDegrees(point, nextPoint);
} else if (prevPoint) {
  headingDeg = computeHeadingDegrees(prevPoint, point);
  pitchDeg = computePitchDegrees(prevPoint, point);
}

rollDeg = computeApproxRollDegrees(prevPoint, point, nextPoint);

if (Math.abs(rollDeg) < 1) {
  rollDeg = 0;
}

const grounded = isGroundedPoint(prevPoint, point, nextPoint);
...
if (grounded) {
  ...
  const heading = Cesium.Math.toRadians(headingDeg + headingOffsetDeg);
...
dashboardHandler = function () {
  ...
  if (nowMs - lastDashboardUpdateMs > 300) {
    lastDashboardUpdateMs = nowMs;
    updateFlightDashboard();
  }
};

viewer.scene.preRender.addEventListener(dashboardHandler);
updateFlightDashboard();
